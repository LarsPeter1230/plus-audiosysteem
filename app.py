...
def api_spotify_source():
    ...
    if request.method == "POST":
        # Bron wisselen is een beheerdersactie (Automix-schakelaar alleen voor admins).
        a = admin_required()
        if a: return a
        j = request.get_json(silent=True) or {}
...
def api_spotify_gui(cmd):
    ...
    r = login_required()
    if r: return r
    a = admin_required()          # Automix-bediening is beheerdersvoorbehouden
    if a: return a
...
def _render_sip_config():
    ...
    cfg = ("module_path      /usr/lib/baresip/modules\n"
           ...
           "auplay_srate     48000\n"            # matcht de dmix-rate (geen extra resample-stap)
           "audio_buffer     60-120\n"           # veilige buffer (geen onderloop/kraak)
           "jitter_buffer_delay  4-8\n"          # veilige jitterbuffer (LAN)
